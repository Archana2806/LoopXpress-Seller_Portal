...
const BusinessDetails: React.FC<{
  ...
  const checkGstInDatabase = async (gstNumber: string) => {
    try {
      const response = await fetch(`${import.meta.env.BACKEND_BASEURL}/api/gst/check-exists`, {
        method: "POST",
        headers: {
  ...
  const verifyGstNumber = async (gstNumber: string) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.BACKEND_BASEURL}/api/gst/verify-gst`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
